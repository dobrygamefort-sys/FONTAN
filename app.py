            ...
            {% if active_chat %}
                ...
                <div class="p-3 border-top" style="background-color: var(--hover-bg);">
                    <div class="d-flex gap-2 align-items-center">
                        ...
                        {% endif %}
                        <button id="emoji-btn" class="btn btn-outline-secondary rounded-circle">рџЉ</button>
...
{% endif %}
<label class="btn btn-sm btn-outline-primary rounded-pill">РР·РјРµРЅРёС‚СЊ С„РѕС‚Рѕ <input type="file" name="avatar" hidden accept="image/*"></label>
</div>
<div class="mb-4 text-center">
...
<select name="theme" class="form-select">
    <option value="light" {% if current_user.theme == 'light' %}selected{% endif %}>вЂпёЏ РЎРІРµС‚Р»Р°СЏ</option>
...
<div class="mt-3 text-center">
  <a href="{{ url_for('sessions') }}" class="text-decoration-none">РСЃС‚РѕСЂРёСЏ РІС…РѕРґРѕРІ Рё СѓСЃС‚СЂРѕР№СЃС‚РІР°</a>
</div>
</div>
</div>
</div> 
...
<div class="card p-3 text-center">
  ...
  {% endif %}
  <div class="text-muted mt-2">РСЃС‚РѕСЂРёСЏ РёСЃС‡РµР·РЅРµС‚: {{ story.expires_at|time_ago }}</div>
</div>
{% endblock %}
""",
    'post_view.html': """
...
<div class="ai-admin-chat-card" id="chatCard{{ chat.id }}">
  <div class="d-flex align-items-center gap-3 mb-2">
    ...
    <div class="ms-auto d-flex gap-2 align-items-center">
      <span class="mode-badge {% if chat.is_admin_mode %}mode-admin{% else %}mode-ai{% endif %}" id="modeBadge{{ chat.id }}">
        {% if chat.is_admin_mode %}рџ‘¤ Р СѓС‡РЅРѕР№{% else %}рџ¤– РР{% endif %}
      </span>
  ...
  <div class="ai-chat-msgs" id="msgs{{ chat.id }}">
    ...
    <div class="ai-admin-msg {{ m.role }}">
      <div style="font-size:.7rem;opacity:.6;margin-bottom:2px;">{{ 'РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ' if m.role == 'user' else 'РР / РђРґРјРёРЅ' }} В· {{ m.timestamp|time_ago }}</div>
...
async function adminReply(chatId) {
  ...
  if(res.ok) {
    ...
    div.innerHTML = `<div style="font-size:.7rem;opacity:.6;margin-bottom:2px;">РР / РђРґРјРёРЅ В· С‚РѕР»СЊРєРѕ С‡С‚Рѕ</div>${text}`;
